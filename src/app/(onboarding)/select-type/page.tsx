...
import { IconArrowRight, IconUser, IconUsersGroup, IconX } from "@tabler/icons-react"
    ...
    return (
        <div>
            <div className="w-full border-b py-4 sticky top-0 bg-white">
                ...
                <h1 className="uppercase font-bold">Student/Organization(Club)</h1>
                <div
          ...
          <div className="flex flex-col gap-1 mx-auto justify-center max-w-[600px] my-4 px-2 space-y-5">
            ...
            <div className="flex gap-2 ">
                <div className={`${ profileType === "student" && 'border-black'} border-2 p-2 rounded-lg cursor-pointer w-full`}  onClick={() => setProfileType("student")}>
                    <IconUser />
                    Student
                </div>
                <div className={`${ profileType === "organization" && 'border-black'} border-2 p-2 rounded-lg cursor-pointer w-full`} onClick={() => setProfileType("organization")}>
                    <IconUsersGroup />
                    Organization/Club
                </div>
            </div>
            <Button label="Continue" rightIcon={<IconArrowRight />} onClick={handleSubmit} />
            </div>           
        </div>
    )
}
